...
m1, m2, m3 = st.columns(3)
m1.metric("Transactions Trained", "6.3M+")
m2.metric("Algorithm", "Logistic Reg.")
m3.metric("Transaction Types", "5")
...
st.caption("Click any example to auto-fill the form")

ex1, ex2, ex3 = st.columns(3)
with ex1:
    if st.button("🟢 Safe Payment", use_container_width=True):
        st.session_state["example"] = "safe"
with ex2:
    if st.button("🔴 Fraud Transfer", use_container_width=True):
        st.session_state["example"] = "fraud1"
with ex3:
    if st.button("🔴 Fraud Cash Out", use_container_width=True):
        st.session_state["example"] = "fraud2"

if "example" in st.session_state:
    if st.session_state["example"] == "safe":
        st.info("✅ Example: Normal PAYMENT of ₹1,000 — Expected: **Not Fraud**")
    elif st.session_state["example"] == "fraud1":
        st.info("⚠️ Example: TRANSFER where sender balance becomes 0 — Expected: **Fraud**")
    elif st.session_state["example"] == "fraud2":
        st.info("⚠️ Example: CASH_OUT draining full balance — Expected: **Fraud**")
...
if st.button("🧠 Predict Transaction", use_container_width=True, type="primary"):
    ...
    safe_prob = round(probability[0] * 100, 1)
    ...
    c1, c2 = st.columns(2)
    c1.metric("🔴 Fraud Probability", f"{fraud_prob}%")
    c2.metric("🟢 Safe Probability", f"{safe_prob}%")

    st.progress(int(fraud_prob), text=f"Fraud risk: {fraud_prob}%")

    with st.expander("View Model & Input Details"):
        st.markdown("**Model Pipeline:** `ColumnTransformer` → `LogisticRegression`")
        st.markdown("**Preprocessing:** StandardScaler (numeric) + OneHotEncoder (categorical)")
        st.dataframe(input_data, use_container_width=True)